WITH raw_data AS (
    SELECT * 
    FROM `datascientiststeamproject.kaggle.Steam`
    WHERE steam_store_available IS TRUE
      -- Fiyat Filtresi: 0.49$ ile 100.00$ arasına çekildi (F2P ve uç değerler elendi)
      AND CAST(price AS FLOAT64) BETWEEN 0.49 AND 100.00
      ...
      -- Toplam yorum sayısı en az 10 olan oyunlar
      AND (COALESCE(CAST(positive AS INT64), 0) + COALESCE(CAST(negative AS INT64), 0)) >= 10
),
